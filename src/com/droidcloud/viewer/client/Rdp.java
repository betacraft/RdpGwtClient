...
public class Rdp {
	...
	/*
	 * droidcloud viewer modifications
	 */

	private static final int INIT_IMAGE_RECT_COUNT = 38;
	private static final int GEN_IMAGE_RECT_COUNT = 4;
	private static int rect_received = 0;
...
//	private RdesktopFrame frame = null;
	...
	static void processBitmapCaps(RdpPacket_Localised data, Options option) {
		...
		// System.out.println("Server bpp: " + bpp);
		...
		// logger.info("setting desktop size and bpp to: " + width + "x" +
		// height
		// + "x" + bpp);
		...
		if (option.getServerDisplayColourDepthInBits() != bpp) {
			...
			// option.getServerDisplayColourDepthInBits()
			// + " to " + bpp);
	...
	public Rdp(Options option, Secure secureLayer) {
		...
		GWT.log("securelayer done");
		this.orders = new Orders();
		GWT.log("order done");
		this.cache = new Cache();
		GWT.log("cache done");
		orders.registerCache(cache);
		GWT.log("cache registered");
		this.option = option;
	...
	public void disconnect() {
		// TODO send disconnect info to outside world
		// DroidCloudViewer: call disconnected from jscript
		// Common.applet.invokeJavaScriptEventHandler("disconnected", null);
		// stopping idle checker and timeout
	...
	private void sendLogonInfo(int flags, String domain, String username,
		...
		if (!option.shouldUseRdp5() || 1 == option.getServerRdpVersion()) {
			...
			data.outUnicodeString("GTB, normaltid",
					2 * "GTB, normaltid".length()); // rdp_out_unistr(s,
													// "GTB, normaltid", 2 *
			...
			data.outUnicodeString("GTB, sommartid",
					2 * "GTB, sommartid".length()); // rdp_out_unistr(s,
													// "GTB, sommartid", 2 *
	...
	private boolean processData(RdpPacket_Localised data, int[] ext_disc_reason)
		...
		case RDP_DATA_PDU_DISCONNECT:
			...
			// logger.debug(("Received disconnect PDU\n"));
			// System.out.println("disconnect pdu");
			return true;
	...
	private void sendGeneralCaps(RdpPacket_Localised data) {
		...
		// data.setLittleEndian16(option.shouldUseRdp5() ? 0x1d04 : 0); // this
		// seems
		/*
		 ...
		 */
		data.setLittleEndian16(0); /* pad2octatesA ignored pg 126 */
		data.setLittleEndian16(0); /* Compression types */
		data.setLittleEndian16(0x0001); /*
										 * A 16-bit, unsigned integer. General
		...
		data.setLittleEndian16(0x0101); /* Pad */
	}
	...
	private void sendBitmapCaps(RdpPacket_Localised data) {
		...
		data.setLittleEndian16(option.getServerDisplayColourDepthInBits()); /*
																			 * Preferred
																			 * BPP
																			 */
		...
		data.setLittleEndian16(option.isBitmapCompressionEnabled() ? 1 : 0); /*
																			 ...
																			 * compression
																			 */
		data.setLittleEndian16(0x0000); /* high colour flag and drawing flags */
		data.setLittleEndian16(0xFFFF); /* multiple ractangle support Pg 129 */
		data.setLittleEndian16(0x0000); /* Pad */
	}
	...
	private void sendOrderCaps(RdpPacket_Localised data) {
		...
		data.setLittleEndian32(Constants.desktop_save ? 0x230400 : 0); /*
																		 * Desktop
																		 * cache
																		 * size
																		 */
		data.setLittleEndian32(0x0000); /* Unknown */
	...
	private void sendBitmapcache2Caps(RdpPacket_Localised data) {
		...
		data.setLittleEndian16(option.isPersistentBitmapCachingEnabled() ? 2
				: 0); /* version */
...
//		Rdesktop.error(exception, frame, true, option);
	...
	private void processPointer(RdpPacket_Localised data)
		...
		int message_type = 0;
		int x = 0, y = 0;
		// System.out.println("Rdp.RDP_POINTER_MOVE");
	...
	private void process_system_pointer_pdu(RdpPacket_Localised data) {
		...
		// System.out.println("RDP.java process_system_pointer: called");
		data.getLittleEndian16(system_pointer_type); // in_uint16(s,
		// system_pointer_type);
		// System.out.print(system_pointer_type);
	...
	protected void processBitmapUpdates(RdpPacket_Localised data)
			throws RdesktopException {
		// //System.out.print("Process bitmap updates called: ");
		...
		for (int i = 0; i < n_updates; i++) {
			...
			if (Bpp == 1) {
				...
				if (pixel != null)
					surface.displayImage(
							Bitmap.convertImage(pixel, Bpp, option), width,
				...
				if (option.getBitmapDecompressionStore() == Options.INTEGER_BITMAP_DECOMPRESSION) {
					...
					// else
					// logger.warn("Could not decompress bitmap");
				} else if (option.BUFFEREDIMAGE_BITMAP_DECOMPRESSION == option
						.getBitmapDecompressionStore()) {
					ImageData pix = Bitmap.decompressImg(width, height, size,
							data, Bpp, option);
					if (pix != null)
		...
		// TODO make changes for repainting the entire deskop
		// surface.repaint(minX, minY, maxX - minX + 1, maxY - minY + 1);

		if (initialized) {

			if (rect_received >= GEN_IMAGE_RECT_COUNT) {
				surface.repaint();
				rect_received = 0;
			} else {
				rect_received++;
			...
			if (rect_received >= INIT_IMAGE_RECT_COUNT) {
				surface.repaint();
				/*
				 * Call for javascript code
				 */
				/*
				 * Common.applet.invokeJavaScriptEventHandler("connected",
				 * null); Common.idleChecker = new IdleChecker();
				 * Common.idleChecker.start();
				 */
				initialized = true;
			} else
				rect_received++;
...
//		surface.setCursor(cache.getCursor(cache_idx));
	 ...
	 */
	public void connect(String server, int flags) throws ConnectionException {
...
//			e.printStackTrace();
		 ...
		 * server); } catch (ConnectException e) { // Handle a refused
		 * connection throw new ConnectionException(
		 * "Connection refused when trying to connect to " + server +
		 * " on port " + option.getPort()); } catch (NoRouteToHostException e) {
		 * // Handle a timeout on connecting throw new ConnectionException(
		 * "Connection timed out when attempting to connect to " + server); }
		 * catch (IOException e) { throw new
		 * ConnectionException("Connection Failed" + e.getMessage()); } catch
		 * (RdesktopException e) { throw new
		 * ConnectionException(e.getMessage()); } catch (OrderException e) {
		 * throw new ConnectionException(e.getMessage()); } catch
		 * (CryptoException e) { throw new ConnectionException(e.getMessage());
		 * }
		 */
	}
	 ...
	 * @param option
	 * @throws com.droidcloud.viewer.RdesktopException
	 * @throws com.droidcloud.viewer.OrderException
	 * @throws com.droidcloud.viewer.crypto.CryptoException
	 */
	...
	public void rdp5_process(RdpPacket_Localised s, boolean encryption,
			boolean shortform, Options option) throws RdesktopException,
