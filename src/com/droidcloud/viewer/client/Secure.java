...
import com.droidcloud.viewer.client.crypto.BlockMessageDigest;
import com.droidcloud.viewer.client.crypto.CryptoException;
import com.droidcloud.viewer.client.crypto.MD5;
import com.droidcloud.viewer.client.crypto.RC4;
import com.droidcloud.viewer.client.crypto.SHA1;
...
public class Secure {
	 ...
	 */
	RdpPacket_Localised _mcs_data;

	public void connect(String host, int port, Rdp rdpLayer)
			throws IOException, RdesktopException, CryptoException,
			OrderException {
	 ...
	 * @throws CryptoException
	 */
	public void connect(String host, Rdp rdpLayer) throws IOException,
	...
	public RdpPacket_Localised sendMcsData() {
		...
		int hostlen = 2 * (option.getHostname() == null ? 0 : option
				.getHostname().length());
		...
		if (option.shouldUseRdp5()) {
			...
			buffer.set8(option.getServerDisplayColourDepthInBits()); // out_uint8(s,
																		// g_server_bpp);
			...
			buffer.setLittleEndian32(option.isConsoleSession() ? 0xb : 0xd); // out_uint32_le(s,
																				// g_console_session
																				// ?
																				// 0xb
																				// :
		...
		// if(option.use_rdp5) buffer.setLittleEndian32(option.encryption ? 0x1b
		// : 0); // 128-bit encryption supported
		// else
		buffer.setLittleEndian32(option.isEncryptionEnabled() ? (option
				.isConsoleSession() ? 0xb : 0x3) : 0);
		...
		if (option.shouldUseRdp5() && (channels.num_channels() > 0)) {
			...
			buffer.setLittleEndian32(channels.num_channels()); // out_uint32_le(s,
																...
																// // number of
																// virtual
			GWT.log("length:"+buffer.getPosition());												// channels
			for (int i = 0; i < channels.num_channels(); i++) {
				// logger.debug(("Requesting channel " +
				// channels.channel(i).name()));
	...
	private void processSrvInfo(RdpPacket_Localised mcs_data) {
		option.setServerRdpVersion(mcs_data.getLittleEndian16()); // in_uint16_le(s,
																	// g_server_rdp_version);
		// logger.debug(("Server RDP version is " +
		// option.getServerRdpVersion()));
	...
	public void establishKey() throws RdesktopException, IOException,
		...
		buffer.copyFromByteArray(this.sec_crypted_random, 0,
				buffer.getPosition(), SEC_MODULUS_SIZE);
