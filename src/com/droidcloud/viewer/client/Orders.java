...
import com.droidcloud.viewer.client.orders.*;
...
public class Orders {	
	...
	public void processOrders(RdpPacket_Localised data, int next_packet,
			int n_orders, Options option) throws OrderException,
			RdesktopException {
		//System.out.println("Process orders called");
		int present = 0;
		// int n_orders = 0;
	...
	private void processRawBitmapCache(RdpPacket_Localised data, Options option)
		...
		cache.putBitmap(cache_id, cache_idx,
				new Bitmap(Bitmap.convertImage(inverted, Bpp, option), width,
						height, 0, 0), 0);
	}
...
//		cache.put_colourmap(cache_id, cm);
	...
	private void processBitmapCache(RdpPacket_Localised data, Options option)
		...
		if (Bpp == 1) {
			...
			if (pixel != null)
				cache.putBitmap(cache_id, cache_idx,
						new Bitmap(Bitmap.convertImage(pixel, Bpp, option),
								width, height, 0, 0), 0);
			/*else
...
*/
	...
	private void processDeskSave(RdpPacket_Localised data,
		...
		if (desksave.getAction() == 0) {
			int[] pixel = surface.getImage(desksave.getLeft(),
					desksave.getTop(), width, height);
	...
	private void processText2(RdpPacket_Localised data, Text2Order text2,
		...
		this.drawText(text2, text2.getClipRight() - text2.getClipLeft(),
			...
			case (0xfe):
				...
				for (int j = 0; j < entry.getSize(); j++) {
					...
					if (glyph != null) {
						...
						surface.drawGlyph(text2.getMixmode(),
								x + (short) glyph.getOffset(), y
										+ (short) glyph.getBaseLine(),
