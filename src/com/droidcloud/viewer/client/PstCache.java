...
public class PstCache {
    ...
    static int g_stamp;
    static String[] g_pstcache_fd = new String[8];
...
//    FileOutputStream fd;
   ...
   //     fd.write(toBigEndian32(stamp), 12 + cache_idx * (g_pstcache_Bpp * MAX_CELL_SIZE + CELLHEADER.size()),4);
        store.setItem(g_pstcache_fd[cache_id]+"/"+cache_idx, new String(toBigEndian32(stamp)));
...
// }
 ...
 static boolean pstcache_put_bitmap(int cache_id, int cache_idx, byte[] bitmap_id, int width, int height, int length, byte[] data, Options option) 
 {
...
/*        fd = new FileOutputStream(g_pstcache_fd[cache_id]);
        ...
        store.setItem(g_pstcache_fd[cache_id]+"/"+cache_idx, new String(data));
//        fd.write(data);
        //rd_lseek_file(fd, cache_idx * (g_pstcache_Bpp * MAX_CELL_SIZE + sizeof(CELLHEADER)));
...
*/
   /* if (!rd_lock_file(fd, 0, 0))
    ...
    g_pstcache_fd[cache_id] = filename;
    return true;
 }
